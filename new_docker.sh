...
#docker-machine ssh default "sudo mount -t cgroup -o none,name=systemd cgroup /sys/fs/cgroup/systemd"#docker-machine ssh default "docker network create --driver bridge --subnet=10.10.10.0/24 --gateway=10.10.10.1 mynet"docker network create --driver bridge --subnet=10.10.10.0/24 --gateway=10.10.10.1 mynet
